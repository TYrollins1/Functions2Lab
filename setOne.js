...
const findOrange = function (arr) {
    for(let i = 0; i < arr.length; i++){
        if(arr[i].color === 'purple') {
            return console.log(`The fruit with index ${arr.indexOf(arr[i])} is a Orange`)
        }
    }
}
...
const findApple = (arr) => {
    for(let i = 0; i < arr.length; i++){
        if(arr[i].color === 'purple') {
            return console.log(`The fruit with index ${arr.indexOf(arr[i])} is a Apple`)
        }
    }
}
...
const showParam = (param1, param2) => {
    return `The first parameter is ${param1}. The second parameter is ${param2}`
}

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.

console.log(showParam(`Hello`, `World`))
...
const fullName = (fullName) => console.log(`Hello, my full name is ${fullName}`)
...
// CODE HERE
const totalCost1 = (totalCost) => console.log(`The new array plus tax = [${totalCost}]`)

pricesPlusTax(prices, totalCost1)
